(function () {
  ...
  function startGeo3D() {
      ...
      function rightsErrorText(kind) {
        if (kind === 'not_found') return 'לא נמצאו נתוני חלקה במאגר.';
      ...
      async function fetchParcelRights(gush, helka) {
        ...
        if (d.error_kind) {
          const err = new Error(rightsErrorText(d.error_kind));
          err.kind = d.error_kind;
          throw err;
        }
      ...
      const parcelRightsHtml = (data) => window.RightsRender.panelHtml(data);
      ...
      function handleParcelRightsClick(button) {
        ...
        fetchParcelRights(g, h).then((d) => {
          ...
          if (seq !== parcelRightsSeq) return;
          const b = document.querySelector('[data-parcel-rights-result]');
          const retry = e.kind === 'not_found' ? '' :
            ' <button class="parcel-plan-action" type="button" data-parcel-rights data-pr-gush="' +
            escapeHtml(String(g)) + '" data-pr-helka="' + escapeHtml(String(h)) + '">נסה שוב</button>';
          if (b) b.innerHTML = '<div class="parcel-plans-result is-error">' + escapeHtml(e.message || 'שגיאה') + retry + '</div>';
        }).finally(() => {
