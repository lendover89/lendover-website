(function () {
  ...
  function startGeo3D() {
      ...
      function planningDetailedCategoryLabel(feature, key) {
        ...
        if (key === 'tama1') {
          const value = normalizedCategoryKey(props.stage || props.type_name || props.Layer_Name || name || 'תמ״א 1');
          if (value.includes('נחל')) return 'נחלים';
          if (value.includes('מוגנ') || value.includes('יער') || value.includes('שמור')) return 'מוגנים';
          if (value.includes('מים') || value.includes('הידרולוג') || value.includes('תהום')) return 'מים';
          if (value.includes('תשת')) return 'תשתיות';
          if (value.includes('פתוח')) return 'שטחים פתוחים';
          return value;
        }
        if (key === 'transport') {
          const text = normalizedCategoryKey([
            props.TypeRoadtx, props.Type_Road, props.Type_Road1, props.TypeRoad_1,
            props.Yeud, props.Ground_TY, props.Tochnit_Ty, props.Tochnit_Nu,
            props.Tochnit_Na, props.Tochnit__1, props.Related, name
          ].filter(Boolean).join(' '));
          if (text.includes('רכבת') || text.includes('מסיל') || Number(props.Type_Code) === 551) return 'מסילות רכבת';
          if (geomType === 'Polygon' || geomType === 'MultiPolygon') return 'רצועות דרך';
          return 'דרכים';
        }
        if (key === 'tama70') {
          const text = normalizedCategoryKey([
            props.MAVAT_NAME, props.TYPE_NAME, props.STAGE, props.Layer_Name,
            props.PLAN_NAME, props.NAME, props.LABEL
          ].filter(Boolean).join(' '));
          if (geomType === 'Point' || geomType === 'MultiPoint' || text.includes('תחנ')) return 'תחנות מטרו';
          if (geomType === 'LineString' || geomType === 'MultiLineString' || text.includes('תוואי')) return 'תוואי מטרו';
          if (text.includes('גבול')) return 'גבול תמ״א 70';
          return 'ייעודי מטרו';
        }
      ...
      function isDrawablePlanningGeometry(feature, key) {
        ...
        if (key === 'tama70' && (type === 'Point' || type === 'MultiPoint')) {
          return planningDetailedCategoryLabel(feature, key) === 'תחנות מטרו';
        }
      ...
      function updatePlanningLegendsVisibility() {
        ...
        const hasDetailedLegend = ['tama1', 'transport', 'tama70'].some((key) => planningLayerEnabled[key]);
        setPlanningLegendVisibility(planningLandUseLegend, planningEnabled && (planningLayerEnabled.landUse || planningLayerEnabled.notice77 || hasDetailedLegend));
      }
      ...
      function setPlanLandUseLegend(items, planNumber) {
        ...
        planningLandUseLegend.innerHTML = '<div class="planning-legend__title">ייעודי קרקע תכנית ' + escapeHtml(planNumber) + '</div>' +
          items.map((item, index) => (
            '<div class="planning-legend__item"><span style="--legend-color:' +
      ...
      function applyDetailedPlanningCategoryFilters(key) {
        const targets = {
          tama1: ['planning-tama1-fill', 'planning-tama1-line'],
          transport: ['planning-transport-fill', 'planning-transport-line'],
          tama70: ['planning-tama70-fill', 'planning-tama70-route-halo', 'planning-tama70-line', 'planning-tama70-stations']
        }[key] || [];
      ...
      async function identifyPlanningAt(e) {
        ...
        const renderedFeature = pickPlanningFeature([...lineFeatures, ...fillFeatures], p);
      ...
      map.on('load', () => {
        ...
        [
          ...
          PLANNING_SOURCES.groundwaterRisk,
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'fill-opacity': ['interpolate', ['linear'], ['zoom'], 11, 0.12, 16, 0.24]
          }
        });
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'fill-opacity': ['interpolate', ['linear'], ['zoom'], 11, 0.1, 16, 0.2]
          }
        });
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'line-width': ['interpolate', ['linear'], ['zoom'], 11, 4.2, 16, 8.5],
            'line-opacity': 0.72
          }
        });
        ...
        map.addLayer({
          ...
          paint: {
            'line-color': [
              'case',
              ['==', ['get', 'planning_category_key'], 'תוואי מטרו'],
              '#22d3ee',
              ['coalesce', ['get', 'planning_category_color'], '#d8b4fe']
            ],
            'line-width': ['interpolate', ['linear'], ['zoom'], 11, 2.4, 16, 5.2],
            'line-opacity': 0.96
          }
        });
