  ...
  function startGeo3D() {
        ...
        return '<div class="parcel-plans-result"><strong>תכניות חלות לפי קווים כחולים</strong>' +
          '<div class="parcel-plans-list">' + plans.map((feature) => {
            ...
            return '<div class="parcel-plan-item"><div class="parcel-plan-title">' +
              ...
              '</div>' + (status ? '<div class="parcel-plan-status">' + escapeHtml(status) + '</div>' : '') + '</div>';
          }).join('') + '</div>' +
          '<span>חיתוך ראשוני לפי גיאומטריית החלקה והקו הכחול. יש לתקף מול מסמכי התכנית.</span></div>';
      }
    ...
    postGeo3DUsage('reset_view');
      ...
      document.addEventListener('click', (event) => {
        const button = event.target.closest('[data-parcel-groundwater], [data-parcel-plans]');
        if (!button) return;
        event.preventDefault();
        if (button.matches('[data-parcel-plans]')) {
