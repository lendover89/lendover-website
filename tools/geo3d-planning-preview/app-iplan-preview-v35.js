(function () {
  ...
  function startGeo3D() {
      ...
      function planningDetailedCategoryLabel(feature, key) {
        ...
        if (key === 'tama1') {
          return normalizedCategoryKey(props.stage || props.type_name || props.Layer_Name || name || 'תמ״א 1');
        }
        if (key === 'transport') {
          return normalizedCategoryKey(props.TypeRoadtx || props.Type_Road || props.Type_Road1 ||
            props.TypeRoad_1 || props.Yeud || props.Ground_TY || props.Tochnit_Ty || name || 'תחבורה');
        }
        if (key === 'tama70') {
          return normalizedCategoryKey(props.MAVAT_NAME || props.TYPE_NAME || props.STAGE ||
            props.Layer_Name || props.PLAN_NAME || name || 'תמ״א 70');
        }
      ...
      function setDetailedPlanningLegend() {
        ...
        ['tama1', 'transport', 'tama70'].forEach((key) => {
          ...
          layerItems.slice(0, 8).forEach((item) => {
            items.push({
              ...item,
      ...
      function rememberDetailedPlanningLegend(key, features) {
        ...
        detailedPlanningLegendByLayer.set(key, Array.from(itemsByLabel.values()));
      ...
      map.on('load', () => {
        ...
        map.addLayer({
          ...
          layout: { visibility: 'none' },
          paint: {
            'line-color': ['coalesce', ['get', 'planning_category_color'], '#d8b4fe'],
