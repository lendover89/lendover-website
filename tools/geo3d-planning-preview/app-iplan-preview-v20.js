(function () {
  ...
  function startGeo3D() {
      ...
      function planningFeatureListHtml(features) {
        const items = (features || []).map((feature) => {
          ...
          return '<li class="planning-panel__plan-item">' +
            ...
            (fid ? '<button class="planning-panel__plan-select" type="button" data-planning-fid="' + escapeHtml(fid) + '">סמן תכנית</button>' : '') +
      ...
      async function selectPlanningBlueLineFromList(fid) {
        ...
        planningPanelBody?.querySelectorAll('[data-planning-fid]').forEach((button) => {
          button.classList.toggle('is-selected', button.dataset.planningFid === String(fid));
        });
