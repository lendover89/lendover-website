(function () {
  ...
  function startGeo3D() {
      ...
      const mapUiScrollSurfaces = document.querySelectorAll(
        '.toggle-group, .search-box, .groundwater-panel, .planning-panel, .planning-legend, .ownership-legend, .status-line'
      );
    ...
    postGeo3DUsage('reset_view');
      ...
      mapUiScrollSurfaces.forEach((surface) => {
        ['touchstart', 'touchmove', 'wheel', 'pointerdown', 'pointermove'].forEach((eventName) => {
