  ...
  function startGeo3D() {
      ...
      function parcelInfoHtml(props, groundwaterState, plansState) {
        ...
        return '<div class="parcel-panel__context">חלקה נבחרת</div>' +
          ...
          '<table>' + rows.map((row) => '<tr><th>' + escapeHtml(row[0]) +
            '</th><td>' + escapeHtml(row[1]) + '</td></tr>').join('') + '</table>' +
          parcelGroundwaterHtml(groundwaterState) + parcelAppliedPlansHtml(plansState) + '</div>';
      }
      ...
      function table5RightsHtml(state) {
        ...
        return '<div class="planning-panel__context">התאמות מטבלה 5</div>' +
          ...
          '<div class="planning-panel__note">הערכים מוצגים בזהירות מתוך טבלה 5. ערכים חריגים מוסתרים כזכויות ומסומנים כגולמיים עד לפענוח מול מסמכי התכנית.</div>';
      }
    ...
    postGeo3DUsage('reset_view');
      ...
      document.addEventListener('click', (event) => {
        const button = event.target.closest('[data-parcel-groundwater], [data-parcel-plans], [data-parcel-planning-fid], [data-parcel-planning-landuse-plan]');
        if (!button) return;
