(function () {
  ...
  function startGeo3D() {
      ...
      function parcelRightsHtml(data) {
        ...
        const confLabel = (c) => c === 'high' ? 'ודאי' : (c === 'medium' ? 'בינוני' : 'לא ודאי — לאימות ידני');
        let html = '<div class="parcel-plans-result"><strong>זכויות בנייה לחלקה' +
          (data.parcel_area_m2 ? ' (' + escapeHtml(String(data.parcel_area_m2)) + ' מ"ר)' : '') +
          '</strong><div class="parcel-plans-list">';
        data.plans.forEach((p) => {
          const title = (p.pl_number ? p.pl_number + ' · ' : '') + (p.pl_name || 'תכנית');
          html += '<div class="parcel-plan-item"><div class="parcel-plan-title">' + escapeHtml(title) + '</div>';
          if (p.status) html += '<div class="parcel-plan-status">' + escapeHtml(p.status) + '</div>';
          if (p.rights_mode === 'h619_methamim' && Array.isArray(p.categories) && p.categories.length) {
            ...
            if (a.rights_above_pct != null) parts.push(a.rights_above_pct + '% בנייה מעל הקרקע');
            if (a.rights_below_pct != null) parts.push(a.rights_below_pct + '% מתחת');
            html += '<div class="parcel-plan-rights"><div>אזור: ' + escapeHtml((dom && dom.category) || '') + '</div>' +
              (parts.length ? '<div>' + escapeHtml(parts.join(' · ')) + '</div>' : '<div>אין ערכים מספריים לאזור זה</div>') +
              '<div class="parcel-plan-status">ודאות: ' + confLabel(p.confidence) +
            ...
            html += '<div class="parcel-plan-rights">' + p.tracks.slice(0, 8).map((t) => {
              const parts = [];
              ...
              return '<div>' + escapeHtml(t.track || '') + (parts.length ? ' · ' + escapeHtml(parts.join(' · ')) : '') + '</div>';
            }).join('') + '</div>';
          } else {
          ...
          if (bn.length) {
            const seen = new Set();
            const labels = bn.map((b) => ((b && (b.label_he || b.label)) || '').trim())
              .filter((l) => l && !seen.has(l) && seen.add(l)).slice(0, 5);
            if (labels.length) html += '<div class="parcel-plan-bonuses">בונוסים: ' + escapeHtml(labels.join(' · ')) + '</div>';
          }
          html += '</div>';
        });
        html += '</div><span>מקור: מאגר זכויות הבנייה. ח/619 מחושב לפי מתחמי התכנית — כיסוי נמוך = לאימות ידני מול נספח המתחמים.</span></div>';
        return html;
      }
