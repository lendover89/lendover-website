  ...
  function startGeo3D() {
      ...
      const PLANNING_CONFIG = {
        ...
        landUse: {
          ...
          url: IPLAN_API + '/Xplan/MapServer/4/query',
          outFields: 'pl_number,pl_name,mavat_name,legal_area,station_desc',
          minZoom: 14
        },
        groundwaterRisk: {
      ...
      async function loadPlanningLayer(key, seq) {
        ...
        const response = await fetch(planningQueryUrl(config, map.getBounds(), 450), {
          credentials: 'include'
        });
        ...
        return { key, count: features.length, exceeded: !!data.exceededTransferLimit };
      }
      ...
      async function loadPlanningLayers() {
        ...
        try {
          ...
          setPlanningPanelHtml('<div class="planning-panel__context">שכבות iPlan באזור הנוכחי</div>' +
            summary + (limited ? '<div class="planning-panel__note">חלק מהשכבות הגיעו למגבלת 450 רשומות. התקרב לאזור קטן יותר לקבלת פירוט מלא.</div>' : '') +
      ...
      function planningFeatureLabel(feature) {
        ...
        if (key === 'landUse') return props.mavat_name || props.pl_name || 'ייעוד קרקע';
      ...
      function planningFeatureHtml(feature) {
        ...
        const rows = key === 'landUse' ? [
          ['שכבה', title],
          ['ייעוד', props.mavat_name],
          ['מספר תכנית', planNumber],
          ['שם תכנית', planName],
          ['שטח חוקי', props.legal_area ? Number(props.legal_area).toLocaleString('he') + ' מ״ר' : '']
        ] : key === 'groundwaterRisk' ? [
          ['שכבה', title],
