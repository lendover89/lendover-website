  ...
  function startGeo3D() {
      ...
      const PLANNING_CONFIG = {
        ...
        transport: {
          ...
          urls: [
            IPLAN_API + '/road_compilation/MapServer/1/query',
      ...
      function updatePlanningLegendsVisibility() {
        ...
        setPlanningLegendVisibility(planningLandUseLegend, planningEnabled && (planningLayerEnabled.landUse || planningLayerEnabled.notice77));
      }
      ...
      function planLandUseType(feature) {
        const props = feature && feature.properties ? feature.properties : {};
        return props.mavat_name || props.TYPE_NAME || props.type_name || props.LABEL || props.NAME || 'ייעוד לא ידוע';
      }
      ...
      function planLandUseColorExpression(types) {
        const expression = ['match', ['coalesce', ['get', 'mavat_name'], ['get', 'TYPE_NAME'], ['get', 'type_name'], 'ייעוד לא ידוע']];
        (types || []).forEach((type, index) => {
          expression.push(type, PLAN_LAND_USE_COLORS[index % PLAN_LAND_USE_COLORS.length]);
        });
        expression.push('#94a3b8');
        return expression;
      }

      function setPlanLandUseLegend(types, planNumber) {
        ...
        planningLandUseLegend.innerHTML = '<div class="planning-legend__title">ייעודי קרקע תכנית ' + escapeHtml(planNumber) + '</div>' +
          types.map((type, index) => (
            ...
            PLAN_LAND_USE_COLORS[index % PLAN_LAND_USE_COLORS.length] +
            '"></span>' + escapeHtml(type) + '</div>'
          )).join('');
      }
      ...
      function planningFeatureLabel(feature) {
        ...
        if (key === 'notice77') return props.PL_NAME || props.pl_name || props.PLAN_NAME || genericName || 'פירוט תכנית';
      ...
      function planningIdentifyPointLayers() {
        const layers = [];
        if (planningLayerEnabled.notice77 && map.getLayer('planning-77-point')) layers.push('planning-77-point');
        if (planningLayerEnabled.tama1 && map.getLayer('planning-tama1-point')) layers.push('planning-tama1-point');
        if (planningLayerEnabled.transport && map.getLayer('planning-transport-point')) layers.push('planning-transport-point');
        if (planningLayerEnabled.tama70 && map.getLayer('planning-tama70-point')) layers.push('planning-tama70-point');
        return layers;
      }
      ...
      function planningFeatureIdentity(feature) {
        ...
        if (props.OBJECTID !== null && props.OBJECTID !== undefined) {
          return [props.planning_layer || '', 'OBJECTID', props.OBJECTID, props.planning_title || ''].join('|');
        }
      ...
      async function showPlanLandUse(planNumber) {
        ...
        const types = Array.from(new Set(features.map(planLandUseType).filter(Boolean))).slice(0, PLAN_LAND_USE_COLORS.length);
        source.setData({ type: 'FeatureCollection', features });
        planningLayerEnabled.notice77 = true;
        updatePlanningLayerVisibility();
        if (map.getLayer('planning-77-fill')) {
          map.setPaintProperty('planning-77-fill', 'fill-color', planLandUseColorExpression(types));
        ...
        setPlanLandUseLegend(types, planNumber);
        updatePlanningLegendsVisibility();
      ...
      async function identifyPlanningAt(e) {
        ...
        const pointLayers = planningIdentifyPointLayers();
        ...
        const pointFeatures = pointLayers.length ?
          map.queryRenderedFeatures([[p.x - 12, p.y - 12], [p.x + 12, p.y + 12]], { layers: pointLayers }) : [];
        ...
        const renderedFeature = pickPlanningFeature([...pointFeatures, ...lineFeatures, ...fillFeatures], p);
