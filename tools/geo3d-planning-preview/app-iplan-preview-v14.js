(function () {
  ...
  function startGeo3D() {
      ...
      const planningRefresh = document.getElementById('planningRefresh');
      ...
      const PLANNING_CONFIG = {
        blueLines: {
          ...
          url: IPLAN_API + '/Xplan/MapServer/1/query',
          outFields: 'pl_number,pl_name,station_desc,pl_area_dunam,pl_landuse_string,pl_url'
        },
        landUse: {
          title: 'ייעודי קרקע',
      ...
      function setPlanningVisibility(isVisible) {
        ...
        if (isVisible) {
          if (hasPlanningLayersEnabled()) {
            setPlanningPanelHtml('טוען שכבות תכנון לפי האזור הנוכחי...');
            schedulePlanningLoad(60);
          } else {
            clearPlanningSources();
            setPlanningHighlight(null);
            setPlanningPanelHtml('בחר שכבת תכנון להפעלה. כברירת מחדל כל השכבות כבויות כדי לשמור על מפה נקייה.');
          }
        } else {
          if (planningPopup) {
      ...
      function planningQueryUrl(config, bounds, count) {
      ...
      function planningQueryUrlFor(config, url, bounds, count) {
        const sw = bounds.getSouthWest();
        const ne = bounds.getNorthEast();
        const params = new URLSearchParams({
          where: '1=1',
          ...
          outSR: '4326',
          f: 'geojson',
          resultRecordCount: String(count || 300)
        });
      ...
      async function loadPlanningLayer(key, seq) {
        ...
        const partsSettled = await Promise.allSettled(urls.map(async (url) => {
          const response = await fetch(planningQueryUrlFor(config, url, map.getBounds(), config.urls ? 250 : 450), {
            credentials: 'include'
          });
        ...
        const features = parts.flatMap((data) => Array.isArray(data.features) ? data.features : []);
        clearPlanningFeatureIndexForLayer(key);
        ...
        return { key, count: features.length, exceeded: parts.some((data) => !!data.exceededTransferLimit) };
      }
      ...
      async function loadPlanningLayers() {
        ...
        try {
          ...
          const summary = results.map((result) => {
            const config = PLANNING_CONFIG[result.key];
            if (result.reason === 'zoom') return config.title + ': זום קרוב יותר';
            return config.title + ': ' + formatCount(result.count || 0);
          }).join('<br>');
          const limited = results.some((result) => result.exceeded);
          setPlanningPanelHtml('<div class="planning-panel__context">שכבות iPlan באזור הנוכחי</div>' +
            summary + (limited ? '<div class="planning-panel__note">חלק מהשכבות הגיעו למגבלת רשומות. התקרב לאזור קטן יותר לקבלת פירוט מלא.</div>' : '') +
            '<div class="planning-panel__note">לחץ על פוליגון תכנון במפה כדי לראות פרטים וקישור למבא״ת.</div>');
        } catch (error) {
      ...
      function identifyPlanningAt(e) {
        ...
        const feature = resolvePlanningSourceFeature(renderedFeature, p);
    ...
    postGeo3DUsage('reset_view');
      ...
      if (planningRefresh) {
        planningRefresh.addEventListener('click', () => {
          if (!planningEnabled) return;
          schedulePlanningLoad(10);
          showStatus('מרענן שכבות תכנון');
        });
      }
