      ...
      async function queryGroundwaterPoint(lngLat) {
        ...
        try {
          ...
          if (response.status === 401) {
            setGroundwaterPanelHtml('נדרשת התחברות כדי לחשב עומק מי תהום.');
            showStatus('נדרשת התחברות');
            return true;
          }
          if (response.status === 429) {
            setGroundwaterPanelHtml('יותר מדי בקשות. נסה שוב בעוד מספר דקות.');
            showStatus('יותר מדי בקשות');
            return true;
          }
          if (!response.ok) {
            setGroundwaterPanelHtml(escapeHtml(data.message || data.error || 'שגיאה בבדיקת מי תהום.'));
            showStatus('שגיאה בבדיקת מי תהום');
            return true;
          }
          ...
          setGroundwaterPanelHtml('לא ניתן להתחבר לשירות מי התהום כרגע.');
