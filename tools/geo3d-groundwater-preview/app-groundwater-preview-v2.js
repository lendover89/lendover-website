(function () {
  ...
  function startGeo3D() {
      ...
      const GROUNDWATER_LAYER_IDS = ['groundwater-coverage-fill', 'groundwater-coverage-line', 'groundwater-marker'];
      ...
      function parcelGroundwaterHtml(state) {
        if (!state) {
          return '<button class="parcel-groundwater-button" type="button" data-parcel-groundwater>בדוק מי תהום</button>';
        }
        if (state.status === 'loading') {
          return '<div class="parcel-groundwater-result is-loading">בודק מי תהום לפי מרכז החלקה...</div>';
        }
        ...
        if (result.coverage === 'none' || result.depth == null) {
          return '<div class="parcel-groundwater-result"><strong>מי תהום</strong>' +
            '<span>אין נתונים זמינים במרכז החלקה.</span></div>';
        }
        const rows = [
          ['עומק משוער', formatMeters(result.depth, 1)],
          ['אקוויפר', result.aquiferDisplay || result.aquifer || '—']
        ];
        if (result.waterLevelYear) rows.push(['שנת מפלס', String(result.waterLevelYear)]);
        return '<div class="parcel-groundwater-result"><strong>מי תהום לפי מרכז החלקה</strong>' +
      ...
      function identifyParcelAt(e) {
        ...
        selectedParcel = {
          props,
          lngLat: parcelCentroid(highlight, e.lngLat),
          groundwater: null
        };
      ...
      async function querySelectedParcelGroundwater() {
        ...
        setGroundwaterPanelHtml('בודק עומק מי תהום לפי מרכז החלקה...');
        showStatus('בודק מי תהום לחלקה', true);
        try {
          const data = await fetchGroundwaterPoint(selectedParcel.lngLat);
          selectedParcel.groundwater = { status: 'done', result: data };
          refreshParcelPopup();
          renderGroundwaterResult(data, selectedParcel.lngLat);
          showStatus('בדיקת מי תהום לחלקה הושלמה');
          postGeo3DUsage('groundwater_parcel_centroid');
        } catch (error) {
          ...
          showStatus('שגיאה בבדיקת מי תהום לחלקה');
        }
      }
