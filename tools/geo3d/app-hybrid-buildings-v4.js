(function () {
  ...
  let statusTimer = 0;
  const TERRAIN_SAFE_MAX_ZOOM = 16;
  ...
  function syncTerrainForZoom(notify) {
    const shouldSuspend = terrainEnabled && map.getZoom() > TERRAIN_SAFE_MAX_ZOOM;
    if (shouldSuspend === terrainSuspendedForZoom) {
    ...
    if (notify) {
      showStatus(
        terrainSuspendedForZoom
          ? 'הטופוגרפיה הוסתרה בזום גבוה כדי למנוע ארטיפקטים של DEM'
          : 'הטופוגרפיה חזרה עם היציאה מהזום הגבוה'
      );
    }
  }
  ...
  map.on('load', () => {
    ...
    map.addLayer({
      ...
      layout: {
        visibility: 'none',
        'text-field': ['coalesce', ['get', 'name:he'], ['get', 'name'], ['get', 'name_en']],
